...
import { Card, CardContent, FormControlLabel, Radio, RadioGroup, TextField } from "@material-ui/core";
...
const VoucherCreatePage: React.FC<VoucherCreatePageProps> = ({
  ...
  const handleRewardTypeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    change(event);
  };
          ...
          <DetailPageLayout.Content>
            ...
            <Card>
              <CardContent>
                ...
                >
                  <FormControlLabel
                    value={VoucherRewardTypeEnum.DISCOUNT}
                    control={<Radio color="primary" />}
                    label={intl.formatMessage({
                      id: "rewardTypeDiscount",
                      defaultMessage: "할인",
                    })}
                    disabled={disabled}
                  />
                  <FormControlLabel
                    value={VoucherRewardTypeEnum.POINTS}
                    control={<Radio color="primary" />}
                    label={intl.formatMessage({
                      id: "rewardTypePoints",
                      defaultMessage: "포인트 지급",
                    })}
                    disabled={disabled}
                  />
                </RadioGroup>
            ...
            />
            {data.rewardType === VoucherRewardTypeEnum.DISCOUNT && (
              <>
            ...
            {data.rewardType === VoucherRewardTypeEnum.POINTS && (
              <>
                <CardSpacer />
                <Card>
                  <CardContent>
                    <TextField
                      fullWidth
                      type="number"
                      name="rewardPoints" // useForm의 'data.rewardPoints'와 연결됩니다.
                      label={intl.formatMessage({
                        id: "rewardPointsLabel",
                        defaultMessage: "지급 포인트",
                      })}
                      value={data.rewardPoints}
                      onChange={change} // useForm의 change 핸들러를 사용합니다.
                      disabled={disabled}
                      inputProps={{
                        min: 1, // 0 이하의 값은 입력하지 못하도록 설정
                      }}
                      // error={...} // 에러 처리 로직 추가 가능
                      // helperText={...}
                    />
                  </CardContent>
                </Card>
