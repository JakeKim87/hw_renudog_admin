...
export const vouchersListStaticColumnsAdapter = (
  ...
  [
    ...
    {
      id: "rewardType",
      title: "리워드 타입", // messages.ts에 추가 필요
      width: 150,
    },
    {
      id: "min-spent",
...
export const createGetCellContent =
  ...
  ([column, row]: Item): GridCell => {
    ...
    switch (columnId) {
      ...
      case "rewardType":
        if (rowData.rewardType === "POINTS") { 
          return readonlyTextCell("포인트");
        }

        return readonlyTextCell("할인");
      case "min-spent":
      ...
      case "value":
        if (rowData.rewardType === "POINTS") {
          const displayValue = rowData.rewardPoints
            ? intl.formatNumber(rowData.rewardPoints)
            : PLACEHOLDER;

          return readonlyTextCell(displayValue);
        }
