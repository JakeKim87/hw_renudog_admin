...
// ▼▼▼ [수정] FormData 타입 정의는 이미 올바르게 수정되어 있습니다. 그대로 둡니다. ▼▼▼
...
const VoucherDetailsPage: React.FC<VoucherDetailsPageProps> = ({
  ...
  const initialForm: VoucherDetailsPageFormData = {
    ...
    rewardType: voucher?.rewardType || VoucherRewardTypeEnum.DISCOUNT,
    rewardPoints: voucher?.rewardPoints ?? "",
  };
  ...
  return (
    <Form confirmLeave initial={initialForm} onSubmit={onSubmit}>
      {({ change, data, submit, triggerChange, set }) => {
        ...
        // ▼▼▼ [추가] RadioGroup의 onChange 타입 에러 해결을 위한 래퍼 함수 ▼▼▼
