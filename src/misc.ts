...
export const transformDeliveryStatus = (
  ...
  switch (status) {
    ...
    case DeliveryStatus.IN_TRANSIT:
      return {
        localized: "배송중",
        status: StatusType.INFO, // 주황색/노란색 계열
      };
    case DeliveryStatus.DELIVERED:
      return {
        localized: "배송 완료",
        status: StatusType.GENERIC, // 초록색 계열
      };
    case DeliveryStatus.DELIVERY_FAILED:
      return {
        localized: "배송 실패",
        status: StatusType.ERROR, // 빨간색 계열
      };
  }
  ...
  return {
    localized: status,
    status: StatusType.ERROR, // 회색 계열
  };
};

export function transformIsActiveStatus(isActive: boolean | undefined | null): {
  localized: string;
