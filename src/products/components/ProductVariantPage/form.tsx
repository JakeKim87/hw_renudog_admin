...
function useProductVariantUpdateForm(
  ...
  const currentChannelsWithPreorderInfo = opts.currentChannels?.map(channel => {
    ...
    return {
      ...
      tiers: variantChannel?.tiers?.map(t => ({
  ...
  const handleUpdateChannels = (selectedIds: string[]) => {
    const allChannels = variant.product.channelListings.map(listing => {
      ...
      if (variantChannel) {
        // [수정] extractChannelPricesFromVariantChannel 함수가 tiers를 반환하는지 확인 필요
        // 만약 해당 함수가 tiers를 반환하지 않는다면 직접 매핑해야 합니다.
        
        // (예시: 직접 매핑)
        const { costPrice, discountedPrice, price } = extractChannelPricesFromVariantChannel(variantChannel);
        
        // [중요] Tiers 매핑 추가
        // variantChannel.tiers는 백엔드에서 받아온 데이터 (GraphQL Fragment에 추가되어 있어야 함)
        const tiers = variantChannel.tiers?.map(t => ({
             minQuantity: t.minQuantity,
             price: t.price.amount // 또는 t.price
        })) || [];
