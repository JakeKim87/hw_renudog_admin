...
export function createGetCellContent({
  ...
  return ([column, row]: Item, { changes, getChangeIndex, added, removed }: GetCellContentOpts) => {
    ...
    switch (columnId) {
      ...
      case "price":
        return getPriceCellContent(rowData);
      case "date":
...
function getPriceCellContent(rowData: RelayToFlat<ProductListQuery["products"]>[number]) {
  // 반환 타입을 TextCell로 명시하면 더 안정적입니다.

  const listing = rowData.channelListings?.[0];
  ...
  const priceRangeStart = listing.pricing?.priceRange?.start?.gross;
  const priceRangeStop = listing.pricing?.priceRange?.stop?.gross;
  ...
  const formattedStartAmount = priceRangeStart.amount.toLocaleString();

  if (priceRangeStop && priceRangeStart.amount !== priceRangeStop.amount) {
    const formattedStopAmount = priceRangeStop.amount.toLocaleString();

    priceString = `${formattedStartAmount}원 ~ ${formattedStopAmount}원`;
  } else {
    priceString = `${formattedStartAmount}원`;
  }

  return readonlyTextCell(priceString);
}
