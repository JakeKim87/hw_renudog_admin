      ...
      <DashboardCard.Content gap={2} display="flex" flexDirection="column">
        ...
        <Box data-test-id="category">
          <Combobox
            // [중요] 크롬/엣지 자동완성 간섭 차단
            autoComplete="off"
            id="product-category-select" // ID 충돌 방지를 위해 고유 ID 부여
            disabled={disabled}
            options={categories}
            value={
              data.category
            ...
            // [중요] onChange 핸들러 안전하게 감싸기
            onChange={(event: any) => {
              // Combobox가 Option 객체({value, label})를 직접 줄 경우와
              // ChangeEvent를 줄 경우를 모두 방어
              const selectedValue = event?.target?.value ?? event?.value;
