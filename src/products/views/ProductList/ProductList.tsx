...
export const ProductList: React.FC<ProductListProps> = ({ params }) => {
  ...
  >(
    ...
    [filterVariables, paginationState, sort],
  );
  ...
  const handleSetSelectedProductIds = useCallback(
    ...
    [products, selectedRowIds, setClearDatagridRowSelectionCallback, setSelectedRowIds],
  );
  ...
  const handleCreateProduct = () => {
    // 로드된 상품 타입 리스트를 가져옵니다.
    const productTypes = mapEdgesToItems(searchDialogProductTypesOpts?.data?.search);

    // 상품 타입이 1개라도 존재하면 0번째 ID로 바로 이동
    if (productTypes && productTypes.length > 0) {
      navigate(
        productAddUrl({
          "product-type-id": productTypes[0].id,
        }),
      );
    } else {
      // 만약 데이터가 아직 로드되지 않았거나 없다면 기존처럼 모달을 띄움 (예외 처리)
