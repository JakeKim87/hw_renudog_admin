...
export interface ChannelPriceAndPreorderData {
  ...
  unitsSold?: number;
  tiers?: Array<{
    minQuantity: string | number; // 폼 입력값은 보통 string으로 관리됨
    price: string | number;
  }>;
}
...
export const createVariantChannels = (
  ...
  if (data) {
    return data?.channelListings?.map(listing => ({
      ...
      discountedPrice: listing.discountedPrice?.amount.toString() || "",
      ...
      tiers:
        listing.tiers?.map(tier => ({
          ...
          price: tier.price.amount.toString(), // 혹은 number면 그대로 사용
        })) || [],
    })) as ChannelPriceData[];
  }
