...
export const unfulfilledOrderListPath = "/unfulfilled-orders";
export const unfulfilledOrderListUrl = (params?: OrderListUrlQueryParams): string => {
  const unfulfilledList = unfulfilledOrderListPath;

  if (params === undefined) {
    return unfulfilledList;
  } else {
    return urlJoin(unfulfilledList, "?" + stringifyQs(params));
  }
};

export const preparingOrderListPath = "/preparing-orders";
export const preparingOrderListUrl = (params?: OrderListUrlQueryParams): string => {
  const preparingList = preparingOrderListPath;

  if (params === undefined) {
    return preparingList;
  } else {
    return urlJoin(preparingList, "?" + stringifyQs(params));
  }
};

export const inTransitOrderListPath = "/in-transit-orders";
export const inTransitOrderListUrl = (params?: OrderListUrlQueryParams): string => {
  const inTransitList = inTransitOrderListPath;

  if (params === undefined) {
    return inTransitList;
  } else {
    return urlJoin(inTransitList, "?" + stringifyQs(params));
  }
};

export const deliveredOrderListPath = "/delivered-orders";
export const deliveredOrderListUrl = (params?: OrderListUrlQueryParams): string => {
  const deliveredList = deliveredOrderListPath;

  if (params === undefined) {
    return deliveredList;
  } else {
    return urlJoin(deliveredList, "?" + stringifyQs(params));
  }
};
