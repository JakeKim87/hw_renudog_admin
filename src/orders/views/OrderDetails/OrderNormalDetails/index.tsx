...
export const OrderNormalDetails: React.FC<OrderNormalDetailsProps> = ({
  ...
  const isOrderFullyRefunded = order?.paymentStatus === PaymentChargeStatusEnum.FULLY_REFUNDED;

  // 2. 가상계좌(VBANK) 여부는 '주 결제(primaryPayment)'를 찾아서 확인해야 합니다.
  //    이 로직은 유효하므로 그대로 사용합니다.
  const primaryPayment = useMemo(() => {
    if (!order?.payments?.length) {
      return null;
    }
    const activePayments = order.payments.filter(p => p.isActive);
  ...
  const paymentMethodType = primaryPayment?.paymentMethodType;

  // 3. 주문 취소 불가능 조건을 정의합니다.
  //    - 배송이 시작되었거나 (fulfilled)
  //    - 주문의 결제 상태가 이미 전액 환불된 경우
  const hasOrderFulfillmentsFulfilled = order?.fulfillments.some(
    fulfillment => fulfillment.status === FulfillmentStatus.FULFILLED,
  );

  // const cannotCancel = hasOrderFulfillmentsFulfilled || isOrderFullyRefunded;
  const cannotCancel = isOrderFullyRefunded;
  ...
  return (
    <>
      ...
      <OrderCancelDialog
        ...
        number={order?.number}
        open={params.action === "cancel" && !cannotCancel}
        paymentMethodType={paymentMethodType}
        onClose={closeModal}
        onSubmit={formData =>
          orderCancel.mutate({
            id,
            refundBankCode: formData.refundBankCode,
            refundAccountNumber: formData.refundAccountNumber,
            refundHolderName: formData.refundHolderName,
          })
        }
      />
