...
const OrderHistory = ({
  ...
  const intl = useIntl();
  // ✅✅✅ [수정 시작] ✅✅✅
  // 1. 노트 입력을 위한 상태를 생성합니다.
  const [noteMessage, setNoteMessage] = useState("");

  // 2. 노트 추가 제출 함수를 만듭니다.
  const handleNoteAdd = () => {
    if (noteMessage.trim() !== "") {
      onNoteAdd({ message: noteMessage });
      // 제출 후 입력 필드를 비웁니다.
      setNoteMessage("");
    }
  };

  // 3. 입력 필드의 변경을 처리하는 함수를 만듭니다.
  // TimelineAddNote 컴포넌트는 Form 컴포넌트의 change 함수와 동일한 시그니처를 기대할 수 있습니다.
        ...
        {history ? (
          <Timeline>
            <TimelineAddNote
              ...
              reset={() => setNoteMessage("")} // reset 함수는 상태를 비우는 함수로 대체
            />
            {history
              .slice()
              .reverse()
