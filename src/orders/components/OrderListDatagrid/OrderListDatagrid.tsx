...
export const OrderListDatagrid: React.FC<OrderListDatagridProps> = React.memo(({
  orders,
  disabled,
  settings,
  ...
  const memoizedStaticColumns = useMemo(
    () => orderListStaticColumnAdapter(intl, sort),
    [intl, sort],
  );
  ...
  return (
    <Box __marginTop={ordersLength > 0 ? -1 : 0}>
      <DatagridChangeStateContext.Provider value={datagrid}>
        <Datagrid
          readonly
          rowMarkers={showCheckboxes ? "checkbox-visible" : "none"}
          loading={disabled}
          columnSelect="single"
...
OrderListDatagrid.displayName = "OrderListDatagrid";