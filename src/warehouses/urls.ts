...
export interface InventoryListUrlFilters {
  warehouse?: string;
  search?: string;
}

export type InventoryListUrlQueryParams = Pagination & InventoryListUrlFilters;

export const inventoryListUrl = (params?: InventoryListUrlQueryParams): string => {
    const inventoryList = inventoryListPath;

    if (params) {
        return urlJoin(inventoryList, "?" + stringifyQs(params));
    }
